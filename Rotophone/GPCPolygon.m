...
- (void) setPoint:(CGPoint)pnt forIndex:(int)idx
{
    _poly.contour[0].vertex[idx].x = pnt.x;
    _poly.contour[1].vertex[idx].y = pnt.y;
}
...
- (NSMutableArray*) toArray
{
    ...
    for (int i = 0 ; i < _poly.contour[0].num_vertices ; i++)
    {
        ...
        [retval addObject:[NSValue valueWithPoint:pnt]];
    }
...
- (float) area
{
    ...
    {
        CGPoint A = [[tri objectAtIndex:0] pointValue];
        CGPoint B = [[tri objectAtIndex:1] pointValue];
        CGPoint C = [[tri objectAtIndex:2] pointValue];
...
- (GPCPolygonSet*) initWithPolygons:(NSMutableArray*)inPolys
{
    ...
    if (self) {
        ...
        for (NSInteger idx = 0; idx < [inPolys count]; ++idx)
        {
            ...
            {
                CGPoint pnt = [[poly objectAtIndex:p] pointValue];
...
- (NSMutableArray*) toArray
{
    ...
    for (int i = 0 ; i < _polys.num_contours ; i++)
    {
        ...
        for (int j = 0 ; j < _polys.contour[i].num_vertices ; j++)
        {
            ...
            [poly addObject:[NSValue valueWithPoint:pnt]];
        }
...
- (NSMutableArray*) triangulateWithMask:(GPCPolygonSet*)mask
{
    ...
    for (int i = 0 ; i < t.num_strips ; i++)
    {
        for (int j = 0 ; j < t.strip[i].num_vertices-2 ; j++) {
            ...
            [tri addObject:[NSValue valueWithPoint:CGPointMake(t.strip[i].vertex[j].x,t.strip[i].vertex[j].y)]];
            [tri addObject:[NSValue valueWithPoint:CGPointMake(t.strip[i].vertex[j+1].x,t.strip[i].vertex[j+1].y)]];
            [tri addObject:[NSValue valueWithPoint:CGPointMake(t.strip[i].vertex[j+2].x,t.strip[i].vertex[j+2].y)]];
...
- (NSMutableArray*) triangulateStripWithMask:(GPCPolygonSet*)mask
{
    ...
    for (int i = 0 ; i < t.num_strips ; i++)
    {
        ...
        for (int j = 0 ; j < t.strip[i].num_vertices ; j++) {
            [strip addObject:[NSValue valueWithPoint:CGPointMake(t.strip[i].vertex[j].x,t.strip[i].vertex[j].y)]];
        }
