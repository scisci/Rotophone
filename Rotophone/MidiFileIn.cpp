...
MidiFileIn :: MidiFileIn( std::string fileName )
{
  ...
  if ( !file_ ) {
    throw "MidiFileIn: error opening or finding file";
  }
  ...
  if ( strncmp( chunkType, "MThd", 4 ) || ( *length != 6 ) ) {
    throw "MidiFileIn: file  does not appear to be a MIDI file!";
  }
  ...
  data = (SINT16 *) &buffer;
  if ( *data < 0 || *data > 2 ) {
    throw "MidiFileIn: the file  format is invalid!";
  }
  format_ = *data;
...
#endif
  if ( format_ == 0 && *data != 1 ) {
    throw "MidiFileIn: invalid number of tracks (>1) for a file format = 0!";
  }
  nTracks_ = *data;
 ...
 error:
  throw "MidiFileIn: error reading from file";
}
...
{
  if ( track >= nTracks_ ) {
    throw "MidiFileIn::getNextEvent: invalid track argument";
  }
...
double MidiFileIn :: getTickSeconds( unsigned int track )
{
  ...
  if ( track >= nTracks_ ) {
    throw "MidiFileIn::getTickSeconds: invalid track argument";
  }
...
unsigned long MidiFileIn :: getNextEvent( std::vector<unsigned char> *event, unsigned int track )
{
 ...
 error:
  throw "MidiFileIn::getNextEvent: file read error!";
  return 0;
}
...
unsigned long MidiFileIn :: getNextMidiEvent( std::vector<unsigned char> *midiEvent, unsigned int track )
{
  ...
  if ( track >= nTracks_ ) {
    throw "MidiFileIn::getNextMidiEvent: invalid track argument";
    return 0;
  }
